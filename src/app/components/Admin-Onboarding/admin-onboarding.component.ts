...
export class AdminOnboardingComponent implements OnInit{
 private api=inject(AdminOnboardingService); loading=false; error=''; message=''; rows:any[]=[]; salesPeople:any[]=[]; selected:any=null; total=0;reviewTab:'documents'|'information'='documents';
 ...
 ngOnInit(){this.load();this.api.salesPeople().subscribe({next:r=>this.salesPeople=r.data||[]});}
 ...
 open(id:number,resetTab=true){this.loading=true;if(resetTab)this.reviewTab='documents';this.api.detail(id).pipe(finalize(()=>this.loading=false)).subscribe({next:r=>this.selected=r.data,error:e=>this.error=e.error?.message||'Unable to open review.'});}
 ...
 confirmDialog(){if(this.dialog==='decision'){this.executeDecision();return;}if(this.dialog==='finalReject'){if(this.dialogRemarks.trim().length<5){this.error='Final rejection remarks must contain at least 5 characters.';return;}this.loading=true;this.api.reject(this.selected.userId,this.dialogRemarks.trim()).pipe(finalize(()=>this.loading=false)).subscribe({next:r=>{this.closeDialog();this.message=r.data.message;this.selected=null;this.load();},error:e=>this.error=e.error?.message||'Rejection failed.'});return;}const call=this.dialog==='approve'?this.api.approve(this.selected.userId,this.selected.rowVersion):this.api.retryCredentialEmail(this.selected.userId);this.loading=true;call.pipe(finalize(()=>this.loading=false)).subscribe({next:r=>{this.closeDialog();this.message=r.data.message;this.selected=null;this.load();},error:e=>this.error=e.error?.message||'Action failed.'});}
