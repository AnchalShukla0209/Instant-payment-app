...
export class AdminOnboardingComponent implements OnInit{
 ...
 decide(kind:'field'|'document',item:any,status:'Approved'|'Rejected'){if(!this.canDecide(item))return;this.pendingKind=kind;this.pendingItem=item;this.pendingStatus=status;this.dialogRemarks='';if(status==='Approved'){this.executeDecision();return;}this.dialog='decision';this.dialogTitle=`Reject ${kind==='document'?this.documentLabel(item.documentType):item.fieldName}`;this.dialogText='A clear rejection remark is mandatory and will be visible to the Sales Person.';}
 ...
 get canActivate(){const documents=this.selected?.documents||[];const fields=this.selected?.fields||[];return this.isReviewable&&documents.length>0&&fields.length>0&&documents.every((item:any)=>item.reviewStatus==='Approved')&&fields.every((item:any)=>item.reviewStatus==='Approved');}
 roleName(type:string){return ({RT:'Retailer',AD:'Distributor',MD:'Master Distributor'} as Record<string,string>)[type]||type||'—';}
 documentLabel(type:string){return type==='Logo'?'Shop Photo':type;}
