...
export class AdminOnboardingComponent implements OnInit{
 private api=inject(AdminOnboardingService);private route=inject(ActivatedRoute);private router=inject(Router);loading=false; error=''; message=''; rows:any[]=[]; salesPeople:any[]=[]; selected:any=null; total=0;detailId=0;reviewTab:'documents'|'information'='documents';
 ...
 private loadDetail(id:number,resetTab=true){this.loading=true;if(resetTab)this.reviewTab='documents';this.api.detail(id).pipe(finalize(()=>this.loading=false)).subscribe({next:r=>this.selected=r.data,error:e=>this.error=e.error?.message||'Unable to open review.'});}
