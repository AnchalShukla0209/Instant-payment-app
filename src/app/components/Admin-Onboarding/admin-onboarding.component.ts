...
export class AdminOnboardingComponent implements OnInit{
 private api=inject(AdminOnboardingService); loading=false; error=''; message=''; rows:any[]=[]; salesPeople:any[]=[]; selected:any=null; total=0;
 ...
 open(id:number){this.loading=true;this.api.detail(id).pipe(finalize(()=>this.loading=false)).subscribe({next:r=>this.selected=r.data,error:e=>this.error=e.error?.message||'Unable to open review.'});}
