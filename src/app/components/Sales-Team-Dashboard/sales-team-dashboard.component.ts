...
export class SalesTeamDashboardComponent implements OnInit {
 private api=inject(SalesTeamOnboardingService);private route=inject(ActivatedRoute);private toastr=inject(ToastrService);private _error='';
 ...
 loading=true;search='';fromDate=this.today();toDate=this.today();status='';page=1;pageSize=10;total=0;rows:OnboardingListItem[]=[];view:ListView='all';showCounts=true;
