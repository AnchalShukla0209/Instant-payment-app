...
export class SalesTeamDashboardComponent implements OnInit {
 ...
 ngOnInit(){this.route.queryParamMap.subscribe(p=>{const view=p.get('view') as ListView|null;if(view&&['draft','review','rejected','approved'].includes(view))this.selectView(view);else this.loadDashboard();});}
