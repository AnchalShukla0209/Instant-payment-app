...
export class SalesTeamDashboardComponent implements OnInit {
 ...
 loading=true;search='';fromDate=this.today();toDate=this.today();status='';page=1;pageSize=10;total=0;rows:OnboardingListItem[]=[];error='';view:ListView='all';
 counts={draft:0,pending:0,rejected:0,approved:0};
 ngOnInit(){this.route.queryParamMap.subscribe(p=>{const requested=p.get('view') as ListView|null;this.view=requested&&['draft','review','rejected','approved'].includes(requested)?requested:'all';this.status=this.view==='draft'?'Draft':this.view==='review'?'Review':this.view==='rejected'?'Rejected':this.view==='approved'?'Approved':'';this.page=1;this.loadDashboard();});}
