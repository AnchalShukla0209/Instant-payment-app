...
export class SalesTeamDashboardComponent implements OnInit {
 private api=inject(SalesTeamOnboardingService);private auth=inject(DistributorAuthService);private router=inject(Router);
 readonly session=this.auth.getSession();loading=true;menuOpen=false;search='';fromDate='';toDate='';status='';page=1;pageSize=10;total=0;rows:OnboardingListItem[]=[];error='';view:ListView='all';
 ...
 selectView(view:ListView){this.view=view;this.status=view==='draft'?'Draft':view==='review'?'Review':view==='rejected'?'Rejected':view==='approved'?'Approved':'';this.page=1;this.menuOpen=false;this.loadList();}
 ...
 logout(){this.auth.clearSession();void this.router.navigate(['/salesteam-login']);}
